...
import { User, Trade, Stock, PortfolioItem, TradeType, MarketData } from '../types';
import { MOCK_STOCKS, STARTING_BALANCE } from '../constants';
...
type Action =
  ...
  | { type: 'CLEAR_ERROR' }
  | { type: 'TRADE_SUCCESS'; payload: { trade: Trade; updatedUser: User; userBeforeTrade: User } }
  | { type: 'TRADE_START' }
...
const initialMarketData = MOCK_STOCKS.reduce((acc, stock) => {
...
const appReducer = (state: AppState, action: Action): AppState => {
  switch (action.type) {
    ...
    case 'AUTH_FAIL':
        return { ...state, error: action.payload, isLoading: false, isAuthenticated: false, currentUser: null };
    case 'CLEAR_ERROR':
...
export const AppProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  // Market Price Simulation Interval
  useEffect(() => {
    const marketInterval = setInterval(() => {
        const newMarketData = { ...state.marketData };
        for (const symbol in newMarketData) {
            const currentPrice = newMarketData[symbol];
            const baseStock = MOCK_STOCKS.find(s => s.symbol === symbol);
            const volatility = (baseStock?.price || 100) * 0.001;
            const fluctuation = (Math.random() - 0.5) * volatility;
            const newPrice = Math.max(0.01, currentPrice + fluctuation);
            newMarketData[symbol] = parseFloat(newPrice.toFixed(2));
        }
...
export const executeTrade = async (dispatch: React.Dispatch<Action>, currentState: AppState, stock: Stock, shares: number, type: TradeType) => {
    ...
    if (type === TradeType.BUY) {
        ...
        const existingHolding = userBeforeTrade.portfolio.find(item => item.stock.symbol === stock.symbol);
        ...
        if (existingHolding) {
            newPortfolio = userBeforeTrade.portfolio.map(item => {
                if (item.stock.symbol === stock.symbol) {
                    ...
                    return { ...item, stock: {...item.stock, price: currentPrice }, shares: totalShares, avgCost: totalValue / totalShares };
                }
                return item;
            });
        } else {
            newPortfolio = [...userBeforeTrade.portfolio, { stock: { ...stock, price: currentPrice }, shares, avgCost: currentPrice }];
        }
        
        const newTrade: Trade = { stock: {...stock, price: currentPrice}, shares, price: currentPrice, type, timestamp: Date.now() };
        updatedUser = { ...userBeforeTrade, balance: newBalance, portfolio: newPortfolio, tradeHistory: [...userBeforeTrade.tradeHistory, newTrade] };
        
        dispatch({ type: 'TRADE_SUCCESS', payload: { trade: newTrade, updatedUser, userBeforeTrade } });

    } else { // SELL
        const existingHolding = userBeforeTrade.portfolio.find(item => item.stock.symbol === stock.symbol);
        if (!existingHolding || existingHolding.shares < shares) {
            dispatch({ type: 'TRADE_FAIL', payload: "You don't own enough shares to sell." });
            return;
        }

        const newBalance = userBeforeTrade.balance + totalCost;
        const newPortfolio = userBeforeTrade.portfolio.map(item => {
            if (item.stock.symbol === stock.symbol) {
