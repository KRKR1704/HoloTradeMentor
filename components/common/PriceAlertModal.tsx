...
import { askAI, getPriceAlertExplanationPrompt } from '../../services/geminiService';
...
export const PriceAlertModal: React.FC<PriceAlertModalProps> = ({ alert, onClose }) => {
        ...
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center p-4 z-50 animate-fade-in">
            <div className="bg-card rounded-lg p-6 w-full max-w-md shadow-xl border border-slate-800 animate-slide-in-up">
                ...
                <p className="text-muted-foreground mb-4">
                    <span className="font-bold text-foreground">{item.stock.symbol}</span> has moved to ${newPrice.toFixed(2)}
                    (<span className={isPositive ? 'text-positive' : 'text-destructive'}>{priceChange.toFixed(2)}</span>)
                </p>
                ...
                <div className="bg-secondary/50 p-3 rounded-lg my-4 border border-slate-700">
                    ...
                    {isLoading ? (
                        <p className="text-muted-foreground text-sm animate-pulse">Analyzing the impact...</p>
                    ) : (
