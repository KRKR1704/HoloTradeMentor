...
const NewsPage: React.FC = () => {
  ...
  const renderLoading = () => (
    <div className="flex flex-col items-center justify-center text-center p-8 bg-card border border-slate-800 rounded-lg">
        ...
        <p className="text-sm text-muted-foreground">Tyche is scanning the latest headlines for you.</p>
    </div>
  );
      ...
      {isLoading ? renderLoading() : error ? renderError() : (
        <div className="space-y-4">
          {articles.map((article) => (
            <div key={article.id} className="bg-card border border-slate-800 p-5 rounded-lg shadow-sm animate-slide-in-up" style={{ animationDelay: `${articles.indexOf(article) * 50}ms`}}>
              <h2 className="text-xl font-bold text-accent">{article.title}</h2>
              <div className="text-xs text-muted-foreground my-2">
                <span>{article.source}</span> &bull; <span>{timeAgo(article.publishedAt)}</span>
              </div>
